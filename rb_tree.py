...
class Node:
    ...
    def __eq__(self, other):
        ...
        return (
            self.value == other.value and self.color == other.color and parents_are_same
        )
...
class RedBlackTree:
    ...
    def add(self, value):
        if not self.root:
            self.root = Node(
                value, color=BLACK, parent=None, left=self.NIL_LEAF, right=self.NIL_LEAF
            )
            self.count += 1
            return
